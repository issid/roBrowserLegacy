/**
 * @module UI/Components/GrfViewer/GrfViewer.js
 *
 * Game File Viewer
 */
define(function (require) {
    ...
    Viewer.init = function init () {
        ...
        // Drag drop the GRF.
        ui.find('#info').on('dragover', function () {
        ...
        // Bind icons events
        ui.on('click', '.directory', onDirectoryClick).on('click', '.audio', onAudioClick).on('click', '.img, .thumb', onImageClick).on('click', '.txt', onTextClick).on('click', '.map', onWorldClick).on('click', '.3d', onObjectClick).on('click', '.gr2', onGrannyClick).on('click', '.fx', onEffectClick).on('contextmenu', '.icon', function (event) {
        ...
        setTimeout(function () {
            Events.process(100);
            moveToDirectory('data/model/', true);
        }, 500);
        // todo remove before implementation of 3dmob
        setTimeout(function () {
            onGrannyClick();
    ...
    function initToolBar () {
        ...
        // Search toolbar
        ui.find('#search').focus(function () { this.value = ''; }).blur(function () { this.value = this.value || 'Search...'; }).keydown(function (event) {
            if (event.which === KEYS.ENTER) {
                var value = this.value.replace(/^\s+|\s+$/g, '');
                if (value.length > 2) {
    ...
    function onAudioClick () {
        ...
        Client.loadFile(path, function (url) {
            ...
            // Show it on a box
            box.css('top', (jQuery(window).height() - 100) / 2).append(jQuery(audio).click(function (event) {
                event.stopPropagation();
            }));
    ...
    function onTextClick () {
        ...
        Client.loadFile(path, function (text) {
            box.css('top', (jQuery(window).height() - 300) / 2).append(
                jQuery('<pre/>').text(text).click(function (event) {
                    event.stopPropagation();
                }).css({
                    background: 'white',
                    width: '500px',
    ...
    var onGrannyClick = function onGrannyClickClosure () {
        ...
        return function onGrannyClick () {
            ...
            var ui = Viewer.ui;
            // todo uncomment before implementation of 3dmob
            // var path  = this.getAttribute('data-path').replace(/\\/g, '/');


            // todo remove before implementation of 3dmob
            var arrGr2 = [
                'data/model/3dmob/empelium90_0.gr2',    // 0
                'data/model/3dmob/guildflag90_1.gr2',   // 1
                'data/model/3dmob/treasurebox_2.gr2',   // 2
                'data/model/3dmob/kguardian90_7.gr2',   // 3
                'data/model/3dmob/aguardian90_8.gr2',   // 4
                'data/model/3dmob/sguardian90_9.gr2',   // 5
            ];
            var path = arrGr2[0];
